...
1. Confirm with `claude plugin list`. System tools (poppler, ffmpeg, Noto CJK fonts) and Python packages install in the background; `/tmp/session-setup.done` exists when they are ready (log: `/tmp/session-setup.log`). If anything is missing, run `CLAUDE_CODE_REMOTE=true .claude/hooks/session-start.sh` and continue; newly installed skills load in the next session, so meanwhile read their SKILL.md directly from `~/.claude/plugins/cache/`.
