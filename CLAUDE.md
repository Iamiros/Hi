...
`.claude/settings.json` declares the plugin marketplaces and `.claude/hooks/session-start.sh` installs them, plus the PDF build tools (pymupdf, playwright, weasyprint, beautifulsoup4), at the start of every web session. Plugins: document-skills, taste-skill, caveman, ponytail, pubmed (official NLM PubMed MCP), ppt-master (natively editable PPTX from PDF/DOCX/Markdown), ui-ux-pro-max (palette, font-pairing, chart and slide design data), humanizer (removes AI-sounding prose), watch (lecture video → frames + transcript; needs ffmpeg/yt-dlp, Whisper only if no captions). Project skills in `.claude/skills/`: medical-image-search (Openverse, Wikimedia Commons, NIH Open-i), plus pubmed-database, literature-review, token-budget-advisor, context-budget (copied from ecc, MIT). ecc itself is deliberately NOT installed: it costs ~41,500 tokens per chat.
1. Confirm with `claude plugin list`. System tools (poppler, ffmpeg, Noto CJK fonts) and Python packages install in the background; `/tmp/session-setup.done` exists when they are ready (log: `/tmp/session-setup.log`); on a fresh container this takes a few minutes. Before any step that needs them, wait for that file (poll every 20 s, up to 10 min). Never re-run the hook while it is still installing. If anything is missing, run `CLAUDE_CODE_REMOTE=true .claude/hooks/session-start.sh` and continue; newly installed skills load in the next session, so meanwhile read their SKILL.md directly from `~/.claude/plugins/cache/`.
...
- The matching document skill: pdf, pptx, docx, xlsx. For PowerPoint output use ppt-master (real editable shapes, charts) together with the pptx skill's QA (LibreOffice render → images). Use ui-ux-pro-max for palette, font-pairing and chart-type decisions inside the course theme.
- humanizer on all prose before review pass B, so explanations read like a good teacher, not like AI. Never let it change facts.
- watch when I send a lecture video or link: extract the transcript and key frames and treat them as lecture material.
