...
PARTS = ['00_front.html','01_toc.html','02_part1.html','03_part2.html',
         '04_part3.html','05_part4.html','06_review.html']
...
def build(outfile, bilingual, label):
    ...
    try:
        ...
        d.set_metadata({
            'title': 'Pathology Laboratory — Session 2: Cell and Tissue Injury (%s)' % label,
            'author': 'Amirhossein Dehghan',
            'subject': 'Normal liver and gastrointestinal tract, cellular swelling, fatty change, hyaline degeneration, mucoid change, pathological calcification',
            'keywords': 'pathology laboratory, reversible injury, cloudy swelling, hydropic degeneration, steatosis, tigroid heart, fat heart, hyalinization, mucoid degeneration, dystrophic calcification',
            'creator': 'WeasyPrint',
        })
...
if __name__ == '__main__':
    ...
    if which in ('en','both'):
        build(os.path.join(outdir,'pathology-lab2-english.pdf'), False, 'Full English Edition')
    if which in ('bi','both'):
        build(os.path.join(outdir,'pathology-lab2-bilingual.pdf'), True, 'Bilingual Edition · English + فارسی')
