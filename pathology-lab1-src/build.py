...
def build(outfile, bilingual, label):
    ...
    try:
        ...
        d.set_metadata({
            'title': 'Pathology Laboratory — Session 1: Introduction (%s)' % label,
            'author': 'Amirhossein Dehghan',
            'subject': 'Laboratory method, normal heart lung and kidney, atrophy, hypertrophy, hyperplasia, metaplasia, specimen description',
            'keywords': 'pathology laboratory, gross specimen, microscopic slide, atrophy, hypertrophy, metaplasia, lipofuscin, hydronephrosis',
            'creator': 'WeasyPrint',
        })
...
if __name__ == '__main__':
    ...
    if which in ('en','both'):
        build(os.path.join(outdir,'pathology-lab1-english.pdf'), False, 'Full English Edition')
    if which in ('bi','both'):
        build(os.path.join(outdir,'pathology-lab1-bilingual.pdf'), True, 'Bilingual Edition · English + فارسی')
