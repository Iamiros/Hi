...
PARTS = ['00_front.html','01_toc.html','02_part1.html','03_part2.html','04_part3.html',
         '05_part3b.html','06_part4.html','07_part5.html',
         '08_questions.html','09_answers.html','10_tables.html','11_glossary.html']
...
def build(outfile, bilingual, label):
    ...
    try:
        ...
        d.set_metadata({
            'title': 'Foundations of Medical Microbiology — Session 1 (%s)' % label,
            'author': 'Amirhossein Dehghan',
            'subject': 'The Microbial World, History of Microbiology, The Bacterial Cell Envelope, External and Internal Structures, Laboratory Identification',
            'keywords': 'medical microbiology, bacterial cell structure, Gram stain, peptidoglycan, endospore, Koch postulates, germ theory',
            'creator': 'WeasyPrint',
        })
...
if __name__ == '__main__':
    ...
    if which in ('en','both'):
        build(os.path.join(outdir,'microbiology-session1-english.pdf'), False, 'Full English Edition')
    if which in ('bi','both'):
        build(os.path.join(outdir,'microbiology-session1-bilingual.pdf'), True, 'Bilingual Edition · English + فارسی')
